            ...
            if char == "{":
                ...
                if brace_count == 0 and start_index != -1:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Failed to parse JSON object at index {start_index}: {json_object}"
                        )
...
class AgenticExecutor:
    ...
    def create_session(self, session_id: Optional[str] = None) -> str:
        ...
        session = AgenticSession(
            id=session_id, context={}, conversation_history=[], active_tools=[], tool_responses=[]
        )
    ...
    def load_mcp_config(self, config_path: Optional[str] = None) -> dict[str, Any]:
        ...
        try:
            ...
            raise DisperslError(f"Invalid MCP config JSON: {e}") from e
    ...
    def update_mcp_tools(self) -> None:
        ...
        logger.info(
            f"Updated MCP tools: {len(self.mcp_tools)} tool(s) from {len(self.mcp_clients)} client(s)"
        )
        ...
        while iteration < max_iterations:
            ...
            try:
                ...
                for line in response.text.splitlines():
                    if line.strip():
                        try:
                            ...
                            # Convert tool dicts to ToolCall objects if present
                            if "tools" in chunk_data and chunk_data["tools"]:
                                chunk_data["tools"] = [
                                    ToolCall(**tool) if isinstance(tool, dict) else tool
                ...
                if tool_calls:
                    ...
                    tool_responses, handover = self._process_tool_calls(tool_calls, session)
    ...
    def _process_tool_calls(
        self, tool_calls: list[ToolCall], session: AgenticSession
        ...
        for tool_call in tool_calls:
            try:
                ...
                try:
                    ...
                    logger.debug(
                        f"Regular JSON parsing failed for {function_name}, trying concatenated JSON parsing"
                    )
                    ...
                    if not parsed_objects:
                        raise Exception(
                            f"Failed to parse arguments as JSON or concatenated JSON: {tool_call.arguments}"
                        ) from e
                    ...
                    if len(parsed_objects) > 1:
                        logger.debug(
                            f"Found {len(parsed_objects)} concatenated JSON objects for {function_name}"
                        )
                ...
                if function_name == "end_session":
                    tool_responses.append(
                        ToolResponse(
                            status="SUCCESS", message="Session ended", tool=function_name, output=""
                        )
                    )
                ...
                if function_name == "handover_task":
                    ...
                    additional_args = {
                        k: v for k, v in args.items() if k not in ["agent_name", "prompt"]
                    }
                ...
                logger.error(
                    f"Error executing tool {tool_call.function.get('name', 'unknown')}: {error}"
                )
                tool_responses.append(
                    ToolResponse(
                        status="FAILURE",
    ...
    def _execute_tool(self, tool_name: str, args: dict[str, Any], session: AgenticSession) -> str:
        """
    ...
    def _execute_built_in_tool(
        self, tool_name: str, args: dict[str, Any], session: AgenticSession
    ) -> str:
        """
    ...
    def parse_text_tool_calls(self, text: str) -> list[ToolCall]:
        ...
        for index, tool_call_text in enumerate(tool_calls_text):
            try:
                ...
                standardized_call = {
                    ...
                    "type": "function",
                    "function": {"name": function_name, "arguments": json.dumps(parsed_args)},
                }

                parsed.append(standardized_call)
                logger.debug(
                    f"Parsed tool call: {function_name} with args: {json.dumps(parsed_args)}"
                )
        ...
        tool_calls = []
        for call in parsed:
            tool_call = ToolCall(function=call["function"], arguments=call["function"]["arguments"])
