...
def serialize_request_data(data: Any) -> dict[str, Any]:
    ...
    try:
        ...
        raise SerializationError(f"Failed to serialize request data: {e}", original_error=e) from e
...
def deserialize_response_data(
    ...
    try:
        ...
        raise SerializationError(
            f"Failed to deserialize response data: {e}", original_error=e
        ) from e
...
def deserialize_datetime(date_string: str) -> datetime:
    ...
    try:
        ...
        raise SerializationError(
            f"Failed to parse datetime: {date_string}", original_error=e
        ) from e
...
def clean_none_values(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in data.items():
        if value is None:
            ...
            cleaned_list = [
                clean_none_values(item) if isinstance(item, dict) else item for item in value
            ]
...
def convert_camel_to_snake(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in data.items():
        ...
        if isinstance(value, dict):
            ...
            converted[snake_key] = [
                convert_camel_to_snake(item) if isinstance(item, dict) else item for item in value
            ]
        else:
...
def convert_snake_to_camel(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in data.items():
        ...
        if isinstance(value, dict):
            ...
            converted[camel_key] = [
                convert_snake_to_camel(item) if isinstance(item, dict) else item for item in value
            ]
        else:
